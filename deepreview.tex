...
Deep neural networks that learn to represent data using layers
of increasing abstract features have dramatically improved the
...
for the weights that define each layer of feature detectors and then
...
each contribute plausibility to a conclusion~\citep{metaphors,rogers}.
...
in  a layered network (Figures~\ref{fig:backprop-box} and~\ref{fig:mlp}). 
...
objective function. However, this is needlessly slow when the 
...
inputs to outputs.  It is much faster to compute the average gradient
...
sigmoid or tanh
non-linearities~\citep{NairRelu,Glorot+al-AI-2011-small,Krizhevsky-2012-small,DahlRelu}.
...
activiy vector in its last hidden layer. The ``decoder''
...
high-level reprsentations of images into captions. GT stands for ``Ground Truth'' and was labeled by humans
