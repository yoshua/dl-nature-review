...
Deep Learning techniques allow a machine composed of multiple processing layers
which define representations of the data that are more abstract for deeper layers.
...
change.  Deep {\em convolutional nets}
have brought about dramatic improvements to process
...
Designing feature extractors that are
...
good at learning appropriate distributed representations. 
%These
...
representations~\citep{Bengio-localfailure-NIPS-2006-small,Bengio-decision-trees10},
...
neural network or RNN~\citep{Cho-et-al-EMNLP2014}. One can observe that
...
the way we normally reason or understand language. RNNs
that represent thoughts as big vectors are more compatible with
...
augment RNNs with a memory module. This includes simplified
version of the LSTM
...
%% very large number of possible states. After neural networks won backgammon
%% olympiads in the 90's~\citep{Tesauro90}, an impressive recent development is
%% combines deep convolutional networks with reinforcement learning in a single system that can learn to play many different video
%% games~\citep{Mnih-et-al-2015}. 
%% Pixels on the screen at several time steps are mapped to the
%% expected future reward of each possible action (assuming some policy is followed
%% in the future) and its only inputs are the
...
thanks to the effort of a forward-looking Canadian funding agency 
(the Canadian Institute for Advanced Research, or CIFAR), with
the introduction of unsupervised learning procedures that could
...
networks with RNNs that use reinforcement learning to decide
where to look.  Combining reinforcement learning with deep learning
has already given impressive results~\citep{Tesauro90,Mnih-et-al-2015}
but much more remains to be explored.
Such systems already outperform passive vision
...
few years deep RNNs will become far better at
